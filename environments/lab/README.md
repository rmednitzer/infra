...
Single-operator iteration on a local KVM host with **local state**. Not
for production. Backend rationale:
[ADR-0003](../../docs/adr/0003-state-backend-strategy.md).

## Prerequisites

- A local KVM host with `libvirtd` running; the `default` storage pool
  and `default` network must already exist
...
Lab defaults: [`terraform.tfvars`](./terraform.tfvars) (non-secret
only). Module API:
[`../../modules/libvirt-vm/README.md`](../../modules/libvirt-vm/README.md).
