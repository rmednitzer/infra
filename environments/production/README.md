...
placeholder. Per [ADR-0003](../../docs/adr/0003-state-backend-strategy.md),
production requires a remote, locked, encrypted state backend.
...
1. Provision an S3-compatible bucket with:
   ...
   - `use_lockfile = true` (OpenTofu 1.10+ native locking; prefer over
     `dynamodb_table`)

2. Edit [`backend.tf`](./backend.tf) — replace the local block with the
   S3 block. The commented example is a starting point.
...
`tofu plan`); automated reconciliation is the `state-reconciliation-loop`
pattern in [`platform-blueprint`](https://github.com/rmednitzer/platform-blueprint).

Reviews assigned via [`../../.github/CODEOWNERS`](../../.github/CODEOWNERS).
